...
- [ ] **AUDIO-06** — Birds, wind and nature; no unnecessary continuous water. Sound depends on scene: hear rain only where it visibly rains (U10, U17 refined by U28/U31). Scene-dependent layers implemented and tested across all five places, including dry → rainy → dry and mute/hidden/zero-nature behavior. Listening balance remains unconfirmed; the initially short rain loop is being lengthened.
...
- README public prose passed `stylint README.md`. Independent rendered review remains active; no physical listening or device acceptance is claimed. The 24-second first rain master is explicitly not the final long-loop solution and is being extended from additional distinct sources.

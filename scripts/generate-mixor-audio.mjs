...
if (!apiKey) {
  console.error("Missing ELEVENLABS_API_KEY. Use --env-file=../red-stamp/.env.");
  process.exit(1);
}
...
const AMBIENCE = [
  ...
  },
  {
    ...
    text:
      "Subtle quiet forest floor ambience for an educational nature game: soft distant air, occasional delicate leaves, one or two very gentle drops, wide and calm, no birds, no insects close to the microphone, no voices, no music, no sudden loud sounds, designed to loop",
    duration: 30,
  },
];
...
async function generateMusic(item) {
  ...
  if (fs.existsSync(dest) && !force) return console.log(`skip music/${item.file}`);
...
async function generateSound(item, directory, kind) {
  ...
  if (fs.existsSync(dest) && !force) return console.log(`skip ${kind}/${item.file}`);
...
async function main() {
  ...
  if (target === "ambience" || target === "all") {
    for (const item of AMBIENCE) await generateSound(item, ambienceDir, "ambience");
  }
